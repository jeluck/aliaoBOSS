...
<body>
	<article class="page-container">
		<form action="<%=path%>/rp?a=A-boss-mod&b=agentlevel_mod&mode2='${reList[0].user_id}'" method="post"
			class="form form-horizontal" id="form-article-add" name="ThisForm">
			 <div class="row cl">
				<label class="form-label col-xs-4 col-sm-2">代理商等级:</label>
				<div class="formControls col-xs-8 col-sm-9">
				<select id="machine_id" name="ss" form="form-article-add">
                <c:forEach var="map" items="${reList}" varStatus="status"> 
				<option value="${map['agent_level']}">${map['agent_level']}级</option>
                </c:forEach>
				</select> 
				</div>
			</div>
