	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<c:forEach var="map" items="${reList}" varStatus="status">
			<tr class="text-c">
				<td>${status.count}</td>
				<td>${map['user_id']}</td>
				<td><a href="javascript:;"  onclick="system_category_edit('图片查看','<%=path %>/rp?p0=A-boss-search&p1=album_list_one&p2=${map['id'] }',800,600)" style="width:50px;height:50px;color:blue;">点击查看图片</a></td>
				<%-- <td>${map['image'] }</td> --%>
				<%-- <td>${map['isHeadpic'] }</td> --%>
				<td>${map['nickname'] }</td>
				<td>${map['status'] }</td>
				...
				<c:if test="${map['status'] == '待审核' }">
				   ...
				   <% if(b.equals("1")){%>
				   <a title="删除" href="javascript:;" onclick="client_del(${map['id']},${map['user_id']})" class="ml-5" style="text-decoration:none;">删除</a>	 
				   <% } %>
				</c:if>
...
function fresh_page(pageIndex) {
	...
	//var name = $("#searchname").val();
		$.ajax({
			cache:true,
			type:"post",
			url: "<%=path %>/rp?a=A-boss-search&b=album_list&p3="+pageIndex+"&p4="+name+"&page=tojson",
			async: true,
			...
			success:function(data){
				...
				for(var i = 0;i<json.length-1;i++){
					...
					if(json[i].status=='待审核'){
						...
						+'<a title="删除" href="javascript:;" onclick="client_del('+json[i].id+')" class="ml-5" style="text-decoration:none;">删除</a>';
					}else if(json[i].status=='未通过'){
						str1='<span style="color:red;">未通过</span><a title="删除" href="javascript:;" onclick="client_del('+json[i].id+')" class="ml-5" style="text-decoration:none;">删除</a>';
					}else if(json[i].status=='已通过'){
						str1='<span style="color:green;">已通过</span><a title="删除" href="javascript:;" onclick="client_del('+json[i].id+')" class="ml-5" style="text-decoration:none;">删除</a>';
					}
					if(json[i].refusal==''){
						str2='<td>无</td>';
					}else{
					...
					content += '<tr class = "text-c">'
							...
							+'<td><a href="javascript:;" onclick="system_category_edit(\'图片查看\',\'<%=path %>/rp?p0=A-boss-search&p1=album_list_one&p2='+json[i].id+'\',\'600\',\'510\')"  style="width:50px;height:50px;color:blue;">点击查看图片</a></td>'
...
function client_del(id){
	...
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: '<%=path%>/rp?p0=A-boss-del&p1=photorecord_del&p2='+id,
			success: function(data){
...
function system_category_edit1(id,userid){
	...
	$.ajax({
		type: 'POST',
		url:'<%=path%>/rp?p0=A-boss-mod&p1=album_checkpass&p2='+id+'&p3='+userid,
		success: function(data){
			/*$(obj).parents("tr").remove();*/
