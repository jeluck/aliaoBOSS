...
<body>
	<article class="page-container">
		<form action="<%=path%>/rp?a=A-boss-mod&b=guanfangfenpei_mod" method="post"
			class="form form-horizontal" id="form-article-add" name="ThisForm">
			<div class="row cl">
			   <input type="hidden" value="${reList[0]['id']}"  name="myid">
			</div>
			<div class="row cl">
				<label class="form-label col-xs-4 col-sm-2">分配任务给红娘:</label>
				<div class="formControls col-xs-8 col-sm-9">
					<select id="matchmaker" name="matchmaker">
					<c:forEach items="${reList1}" var="map" varStatus="status">
					    <option value="${map['id'] }">${map['nickname']}</option>
					</c:forEach>
					</select>
				</div>
			</div>
