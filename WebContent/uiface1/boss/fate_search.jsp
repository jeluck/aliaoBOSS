...
<div class="page-container">
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		<thead>
			<tr class="text-c">
				<th width="30">序号</th>
				<th width="40">申请人ID</th>
				<th width="40">申请人昵称</th>
				<th width="40">申请人联系方式</th>
		...
		<c:forEach var="map" items="${reList}" varStatus="status">
			<tr class="text-c">
				...
				<th>
					<c:choose>
						<c:when test="${map['state']==1 }"><span>审核通过</span></c:when>
						<c:when test="${map['state']==0 }"><span>未审核</span></c:when>
					</c:choose>
				</th>
				<td class="td-manage">
...
function fresh_page(pageIndex) {
	//var name = document.getElementById("searchname").value;
		$.ajax({
			cache:true,
			type:"post",
			url: "<%=path %>/rp?a=A-boss-search&b=yuanfenqingqiu&p3="+pageIndex+"&page=&p5=tojson",
			async: true,
...
function system_category_edit1(id,userid){
	...
	$.ajax({
		type: 'POST',
		url:'<%=path%>/rp?p0=A-boss-mod&p1=yuanfenqingqiu_mod&p2='+id+'&p3='+userid,
		success: function(data){
			/*$(obj).parents("tr").remove();*/
