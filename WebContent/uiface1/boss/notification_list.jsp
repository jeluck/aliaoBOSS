...
<nav class="breadcrumb">
	...
	<span class="c-gray en">&gt;</span> 系统消息 
		...
		<tbody id="list-content">
			<c:forEach var = "map" items="${reList }" varStatus="status">
				<tr class="text-c">
					...
					<td>${map['time']}</td>
					<c:choose>
					   <c:when test="${map['status']=='0'}"><td style="color:red;">未发布</td></c:when>
					   <c:when test="${map['status']=='1'}"><td style="color:green;">已发布</td></c:when>
					</c:choose>
...
function fresh_page(pageIndex){
	$.ajax({
		cache: true,
		type: "POST",
		url: "<%=path%>/rp?p0=A-boss-search&p1=notification_search&p2="+pageIndex+"&p4=&p3=tojson" ,
		async: true,
...
function go(){
	age = prompt("请输入要发布的内容","");
	layer.confirm('确认输入以上内容？',function(index){ 

		$.ajax({
			type:'POST',
			url: '<%=path%>/rp?p0=A-boss-add&p1=notification_add&p2='+age,
			success: function(data){
...
function client_del(id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: '<%=path%>/rp?mode1=A-boss-del&mode2=del_xitong_one&p2='+id,
			success: function(data){
				/*$(obj).parents("tr").remove();*/
