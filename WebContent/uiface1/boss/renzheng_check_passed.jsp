...
<nav class="breadcrumb">
	...
	<span class="c-gray en">&gt;</span> 已通过 <!-- 文章列表  --><a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<c:forEach var="map" items="${reList}" varStatus="status">
			<tr class="text-c">
				...
				<td>${map['label'] }</td>
				<td>${map['signature'] }</td>
				<td style="color:green;">${map['status'] }</td>
				<td>${map['time'] }</td>
			</tr>
		</c:forEach>	
		</tbody>
...
function fresh_page(pageIndex) {
	//var name = document.getElementById("searchname").value;
		$.ajax({
			cache:true,
			type:"post",
			url: "<%=path %>/rp?a=A-boss-search&b=renzheng_v_passed&p3="+pageIndex+"&page=tojson",
			async: true,
			...
			success:function(data){
				...
				for(var i = 0;i<json.length-1;i++){
					...
					content += '<tr class = "text-c">'
						    ...
						    +'<td style="color:green;">'+json[i].status+'</td>'
						    +'<td>'+json[i].time+'</td>'
						    ;
