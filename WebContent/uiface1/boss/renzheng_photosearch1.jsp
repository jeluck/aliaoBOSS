...
<link href="<%=path%>1/boss/lib/lightbox2/2.8.1/css/lightbox.css" rel="stylesheet" type="text/css"/>
<!--[if IE 6]>
...
<div class="page-container">
	<div class="portfolio-content">
		<input value="${reList[0].pictures}" type="hidden" id="url"></input>
		<input value="${reList[0].id}" type="hidden" id="userid"/>
		<input  type="hidden" id="oldurl"/>
		<ul class="cl portfolio-area">
			<c:set var="a" value="${reList[0].pictures}" ></c:set>
...
function client_del(delurl){
	...
	layer.confirm('确认要删除吗？',function(index){
		...
		$.ajax({
			type: 'POST',
			url: '<%=path%>/rp?p0=A-boss-mod&p1=anchor_pictures&p2='+id+'&p3='+imgsrc,
