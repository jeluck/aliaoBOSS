...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> 
		<button type="submit" id="thisTyrants" name="" class="btn btn-primary radius">本周土豪榜</button>
		
		<button type="submit" id="todayTyrants" name="" class="btn btn-primary radius">今日土豪榜</button>
...
$(function(){
	$("#thisTyrants").click(function(){
		var i = 0;
		fresh_page(i);
	});
	$("#todayTyrants").click(function(){
		var i = 1;
		fresh_page(i);
	});
});
...
function fresh_page(z){
	$.ajax({
		cache: true,
		type: "POST",
		url: "<%=path%>/rp?p0=A-boss-search&p1=tyrants_week_search&p2="+z+"&p3=tojson",
		async: true,
		...
		success: function(data){
			...
			for(var i = 0; i < json.length; i++) {
				if(z==0){
					a=json[i].tyrants_thisweek;
				}else{
					a=json[i].tyrants_today;
				}
