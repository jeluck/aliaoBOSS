...
<%-- href="<%=path%>/rz?p0=A-boss-search&p1=Wealth_value_search&p2=${map['id']}&p3=1&p4=&p5=&p6=tojsp" --%>
function fresh_page(pageIndex) {
	...
	$.ajax({
		...
		url:"<%=path%>/rp?p0=A-boss-search&p1=Wealth_value_search&p2=${param.p2}&p3="+pageIndex+"&p4="+startdate+"&p5="+enddate+"&p6=tojson",
		async: true,
		...
		success: function(data) {
			...
			for(var i = 0; i < json.length-1; i++) {
				content +='<tr class="text-c">'
					...
					+'<td>'+json[i].pay_price+'</td>'
					/* +'<td>'+json[i].receiver_id+'</td>' */
