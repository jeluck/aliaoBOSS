...
<title>代理商相关比例设置</title>
</head>
<body>
<nav class="breadcrumb">
	<i class="Hui-iconfont">&#xe67f;</i> 首页 
	<span class="c-gray en">&gt;</span> 代理商管理 
	<span class="c-gray en">&gt;</span> 代理商相关比例设置
...
<div class="page-container">
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		<thead>
			<tr class="text-c">
				<th width=20>序号</th>
				<th width="20">代理商等级</th>
				<th width="40">代理商对应比例</th>
				<th width="40">操作</th>
		...
		<tbody id="list-content">
			<c:forEach var = "map" items="${reList }" varStatus="status">
				<tr class="text-c">
					...
					<td><a title="修改" href="javascript:;"onclick="go(${map['id']})" class="ml-5" style="text-decoration: none"><i class="Hui-iconfont">&#xe6df;</i></a></td>
				</tr>
			</c:forEach>
...
function fresh_page(pageIndex){
	...
	$.ajax({
		cache: true,
		type: "POST",
		url: "<%=path%>/rp?p0=A-boss-search&p1=agent_set&p2="+pageIndex+"&p4=&p3=tojson" ,
		async: true,
		...
		success: function(data){
			...
			for(var i = 0; i < json.length-1; i++) {
				 ...
				 content +='<tr class="text-c">'
					...
					+'<td>'+json[i].agent_percent+'</td>'
					+'<td><a href="javascript:;" onclick="go('+json[i].id+')>点击查看图片</span></a></td>'
				; 
			}
...
function go(id){
	...
	layer.confirm('确认输入以上内容？',function(index){
		$.ajax({
			type:'POST',
			url: '<%=path%>/rp?p0=A-boss-mod&p1=agent_mod&p2='+id+'&p3='+age,
			success: function(data){
