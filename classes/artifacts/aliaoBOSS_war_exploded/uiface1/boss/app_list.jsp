...
<nav class="breadcrumb">
	...
	<span class="c-gray en">&gt;</span> app用户分析 
...
function fresh_page(pageIndex){
	$.ajax({
		cache: true,
		type: "POST",
		url: "<%=path%>/rp?p0=A-boss-search&p1=app_search&p2="+pageIndex+"&p4=&p3=tojson" ,
		async: true,
		...
		success: function(data){
			...
			for(var i = 0; i < json.length-1; i++) {
				 ...
				 content +='<tr class="text-c">'
					...
					+'<td>'+json[i].totle_num+'</td>'
					+'<td>'+json[i].active_num+'</td>'
					+'<td>'+json[i].new_num+'</td>'
					; 
			}
