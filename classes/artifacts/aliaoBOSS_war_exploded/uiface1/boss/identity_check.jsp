	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<c:forEach var="map" items="${reList}" varStatus="status">
			<tr class="text-c">
				...
				<td>${map['nickname']}</td>
				<td>${map['rz_type'] }</td>
				<td><img alt="图片加载失败" src="../img/imgheadpic/${map['rz_photo']}" style="width:50px;height:50px"></td>
				<td>${map['rz_status'] }</td>
				<td>${map['rz_time'] }</td>
				<td class="td-manage">
				   <a title="通过" href="javascript:;" onclick="system_category_edit1(${map['id1']})" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">通过</i></a>  	 
				 <a title="不通过" href="javascript:;" onclick="go(${map['id1']})" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">不通过</i></a>	 
				</td>
			</tr>
		</c:forEach>	
		</tbody>
...
function fresh_page(pageIndex) {
	//var name = document.getElementById("searchname").value;
		$.ajax({
			cache:true,
			type:"post",
			url: "<%=path %>/rp?a=A-boss-search&b=identity_check&p3="+pageIndex+"&page=tojson",
			async: true,
...
function system_category_edit1(id){
	...
	$.ajax({
		type: 'POST',
		url:'<%=path%>/rp?p0=A-boss-mod&p1=rz_checkpass&p2='+id,
		success: function(data){
			/*$(obj).parents("tr").remove();*/
...
function go(id){
	...
	layer.confirm('确认输入以上内容？',function(index){
		...
		$.ajax({
			type: 'POST',
			url: '<%=path%>/rp?p0=A-boss-mod&p1=check_nopass&p2='+id+'&p4='+userid+'&p3='+age,
			success: function(data){
	...
	$.ajax({
		type: 'POST',
		url:'<%=path%>/rp?p0=A-boss-mod&p1=rz_checknopass&p2='+id,
		success: function(data){
			/*$(obj).parents("tr").remove();*/
			layer.msg('操作成功',{icon:1,time:1000});
			setTimeout(function () { 
